// ArticleDetail.tsx (Revisi Final dengan Log Verifikasi ID Sumber)
...
                  <a href={cleanAndValidateUrl(coverImage)} download={`fitapp_${slugValue}_cover.jpg`} className="block w-full h-full" target="_blank" rel="noopener noreferrer">
                    <div className="aspect-[16/9] bg-gray-100 animate-pulse">
                      <img 
                        src={cleanAndValidateUrl(coverImage)} 
                        alt={title} 
...
                      />
                      
                      {/* MID GALLERY (Ditampilkan setelah paragraf tertentu) */}
                      {originalIndex === 2 && midGallery.length > 0 && (
                        <div className="my-10 -mx-6 px-6">
                          <h2 className="text-xl font-bold mb-4 text-gray-900 px-6 sm:px-0">{t("Gallery")}</h2>
                          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            {midGallery.map((url: string, i: number) => (
                              <Card key={i} variant="shadow" className="p-0 aspect-[4/5] overflow-hidden">
                                <a href={cleanAndValidateUrl(url)} download={`fitapp_${slugValue}_mid_${i + 1}.jpg`} className="block w-full h-full" target="_blank" rel="noopener noreferrer">
                                  <img src={cleanAndValidateUrl(url)} loading="lazy" className="w-full h-full object-cover !m-0 transition-transform duration-300 hover:scale-[1.03]" alt={`${t("Gallery image")} ${i + 1}`} />
                                </a>
                              </Card>
                            ))}
                          </div>
...
                <h2 className="text-xl font-bold mb-4 text-gray-900">{t("More Photos")}</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {bottomGallery.map((url: string, i: number) => (
                    <Card key={i} variant="shadow" className="p-0 aspect-[4/5] overflow-hidden">
                      <a href={cleanAndValidateUrl(url)} download={`fitapp_${slugValue}_bottom_${i + 7}.jpg`} className="block w-full h-full" target="_blank" rel="noopener noreferrer">
                        <img src={cleanAndValidateUrl(url)} loading="lazy" className="w-full h-full object-cover !m-0 transition-transform duration-300 hover:scale-[1.03]" alt={`${t("Gallery image")} ${i + 7}`} />
                      </a>
                    </Card>
                  ))}
                </div>
